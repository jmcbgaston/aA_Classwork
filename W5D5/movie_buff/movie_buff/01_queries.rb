...
def harrison_ford
  ...
  # appeared but not as a lead actor
    ...
    # Actor.joins(:movies, :castings).select(title: {name: "Harrison Ford"}).where.not("ord = 1")

    Movie
      .select(:id, :title)
      .joins(:actors) # calling actors associaton in the movie model
      .where(actors: {name: "Harrison Ford"})
      .where.not(castings: {ord: "1"})
    
      #Movie.joins(:castings) == FROM movies JOIN castings ON movies.id = castings.movie_id
      #Movie.joins(:actors) == 
      # FROM 
      #   movies 
